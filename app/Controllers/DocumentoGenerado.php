...
class DocumentoGenerado extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->idUsuario =  $this->idUsuario = session()->get('id_usuario');
    ...
    */
    ...
    public function crear()
    {
        ...
        */
        ...
        $validation = $this->validate('documentoGenerado');
        $idNuevoDocumento  = 0;
        $path  = "";

        if ($validation) {
            $asignadoPor = $this->idUsuario;

            $horaEmision = $this->request->getPost('hora_emision');
            $fechaEmision = $this->request->getPost('fecha_emision');

            $timeStampEmision = $fechaEmision . " " . $horaEmision;
            ...
            $documentoDigital = $this->request->getFile('documento_generado');

            if ($documentoDigital->isValid() && !$documentoDigital->hasMoved()) {
                //$documentoDigital->move('./documents/', 'renameFile.' . $documentoDigital->getExtension()); //No hay necesidad de verificar si existe o no el archivo y se renombra el archivo
                $documentoDigital->move('./documents/' . $monthFolder, $documentoDigital->getRandomName()); // la misma operación anterior pero renombrando el archivos 
                //combinacion de time stamp y random string
    ...
    public function reiniciarTiposUsuarios()
    {
        /*Este método reiniciara las columnas que se utilizan para 
      detectar los usuarios que han sido buscados en cualquier 
      select 2 del formulario de generados y poderlos mostrar 
      en caso de que existan validaciones sin cumplierse durante
      la captura. Este es llamado desede el archivo propios.js, mediante ajax
       en el método document ready.  
     */
        $this->usuarios->set([
            'es_remitente' => 0,
            'es_destinatario' => 0,
        ...
        return json_encode(['success' => 'reiniciados']);
    }
}
